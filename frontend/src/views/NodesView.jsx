...
function NodeRow({ node, selected, toggleNode, openLeaseDrawer, releaseLease }) {
  ...
  return <tr className={selected ? 'selected-row' : ''}>
    ...
    <td>{node.usedTelemetryGpus || 0} busy / {node.telemetryGpus?.length || node.gpuCount} seen<small>{node.avgUtilization || 0}% avg SM · {formatNodeMemory(node)}</small></td>
...
function formatNodeMemory(node) {
  if (!node.memoryTotalMb) return '0 GB / 0 GB';
  return `${(node.memoryUsedMb / 1024).toFixed(1)} GB / ${(node.memoryTotalMb / 1024).toFixed(1)} GB`;
}
